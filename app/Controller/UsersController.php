<?php
	...
	class UsersController extends AppController
	{
		...
		public function profile($id = 0)
		{
			...
			$d['Timeline'] = $this->Visited->find('all', array(
				'conditions' => array('User.id' => $d['User']['id']),
				'fields' => array(
			...
			foreach($d['Timeline'] as $k => $v)
			{
				$d['Timeline'][$k] += $this->PlaceComment->find('first', array(
			...
			foreach($d['Timeline'] as $k => $v)
			{
				$d['Timeline'][$k] += $this->Mark->find('first', array(
					'conditions' => array('user_id' => $v['User']['id'], 'place_id' => $v['Place']['id']),
