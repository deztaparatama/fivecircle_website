<?php
	...
	class MobileController extends AppController
	{
		...
		public function request()
		{
			...
			$url = "http://prod.eyesnap.fr/iota/request/";
			$cookieFile = tempnam("/tmp", "zCookie");
			
			$login = "fivecircle";
			$password = "dccyijsce";
			$postAuthentification = array("login" => $login, "passwd" => $password);

			if (($req = curl_init($url)))
			{
				curl_setopt($req, CURLOPT_POST, true);
				curl_setopt($req, CURLOPT_RETURNTRANSFER, true);
				curl_setopt($req, CURLOPT_COOKIEJAR, $cookieFile);
				curl_setopt($req, CURLOPT_POSTFIELDS, $postAuthentification);
				
				if (!($output = curl_exec($req)))
				{
					echo("Cannot exec the authentication request.");
					exit();
				}
			...
			$key="4";
			$postKey = array('key' => $key);
			
			if (($req = curl_init($url)))
			{
				curl_setopt($req, CURLOPT_POST, true);
				curl_setopt($req, CURLOPT_RETURNTRANSFER, true);
				curl_setopt($req, CURLOPT_COOKIEFILE, $cookieFile);
				curl_setopt($req, CURLOPT_POSTFIELDS, $postKey);

				if (!($output = curl_exec($req)))
				{
					echo("Cannot exec the key request.");
					exit();
				}

				curl_close($req);
				echo("Key (".$key.") sent.<br />");
			}
			else
			{
				echo("Connection fails.");
			...
			$cid = $outputExploded[5];
			//$cid="4B7CC5694DD3A265BAC326EABA31266E13587879185F2BEDEEFC1D91432A330EA01820342E";
			$image="/Users/chapa/Desktop/IMAG0512.jpg";
			$postCid = array("cid" => $cid, "image" => "@".$image.";type=image/jpeg");
			...
			if (($req = curl_init($url)))
			{
				...
				curl_setopt($req, CURLOPT_RETURNTRANSFER, true);
				curl_setopt($req, CURLOPT_COOKIEFILE, $cookieFile);
				curl_setopt($req, CURLOPT_POSTFIELDS, $postCid);

				if (!($output = curl_exec($req)))
				{
					echo("Cannot exec the image  request.");
					exit();
				}
				
				echo("Cid (".$cid.") sent - IOTA result:");
				echo($output);
				curl_close($req);
				//echo("Cid (".$cid.") sent.");
			}
